...
accl_y = importdataMat([dir 'right-wrist-accely.csv'], -1);
y800 = importdataMat([dir 'org.md2k.puffmarker.data.accl.y.mag_800' wrist '.csv'], -1);
y8000 = importdataMat([dir 'org.md2k.puffmarker.data.accl.y.mag_8000' wrist '.csv'], -1);

% accl_y=accl_y(find(accl_y(:,2)<2 & accl_y(:,2)>-2));
plot(accl_y(:,1),9+ accl_y(:, 2));
hold on;
plot(y800(:,1),9+ y800(:, 2), '.');
hold on;
plot(y8000(:,1),9+ y8000(:, 2), '.');
hold on;
plot([accl_y(1,1) accl_y(end,1)],[9 9],'k--')

% rolls= importdata([dir 'org.md2k.puffmarker.data.wrist.roll.rightwrist.csv']);
rollPitchOkay= importdataMat([dir 'org.md2k.puffmarker.data.wrist.roll-pitch.okay' wrist '.csv'], -1);
hold on;
plot(rollPitchOkay(:,1),9+ rollPitchOkay(:, 2), '.');
hold on;

rip = importdataMat([dir 'org.md2k.puffmarker.data.rip.csv'], -1);
plot(rip(:,1),6+ rip(:, 2)/2000);
hold on;
features = importdataMat([dir 'org.md2k.puffMarker.fv.csv'], -1);

plot(features(:,1),6+features(:,6)/2000, 'ob'); hold on;

peak= importdataMat([dir 'org.md2k.puffmarker.data.rip.peaks.csv'], -1);
plot(peak(:,1),6+ peak(:, 2)/2000, '.r');
hold on;
valley= importdataMat([dir 'org.md2k.puffmarker.data.rip.valleys.csv'], -1);
plot(valley(:,1),6+ valley(:, 2)/2000, '.g');
hold on;

gyro_mag = importdataMat([dir 'org.md2k.puffmarker.data.gyr.mag' wrist '.csv'], -1);
gyro_mag_800 = importdataMat([dir 'org.md2k.puffmarker.data.gyr.mag_800' wrist '.csv'], -1);
gyro_mag_8000 = importdataMat([dir 'org.md2k.puffmarker.data.gyr.mag_8000' wrist '.csv'], -1);

x = importdataMat([dir 'org.md2k.puffmarker.data.candidate.intersections' wrist '.csv'], -1);
xall = importdataMat([dir 'org.md2k.puffmarker.data.candidate.intersections.nofilter.' wrist '.csv'], -1);
% xall = importdata([dir 'org.md2k.puffmarker.data.gyr.intersections.rightwrist.csv']);
% org.md2k.puffMarker.smoking.episode

fv=importdataMat([dir 'org.md2k.puffMarker.fv.minute.csv'], -1);
epi=importdataMat([dir 'org.md2k.puffMarker.smoking.episode.csv'], -1);
...
gt=importdataMat([dir 'puffGroundtruth.csv'], -1);
% gt=importdata([dir 'PUFF_LABEL_CLOUD.csv']);
p_label=importdataMat([dir 'org.md2k.puffMarker.pufflabel.csv'], -1);
