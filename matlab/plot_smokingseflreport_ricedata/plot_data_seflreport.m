...
for i=1:length(sessions)
    ...
    if length(selfReport)>0
        for i=1:length(selfReport(:, 1)) 
           rectangle('Position',[selfReport(i,1)-15*60000,0,20*60000,5.5],'FaceColor',[.9 1 0.1]); hold on;
           plot([selfReport(i,1) selfReport(i,1)], [0 8], 'k');
           text(selfReport(i,1), 8, 'Self-report');
        end
    end
